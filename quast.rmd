...
metrics <- quast_orig %>%
	...
	mutate(Assembly = Assembly %>%
			...
			str_replace("\\.(hg004|HNJJKCCXX).*breaktigs", "+Tigmint") %>%
			str_replace("\\.(hg004|HNJJKCCXX).*abyss_scaffold", "+ARCS") %>%
			str_replace("\\.(hg004|HNJJKCCXX).*links", "+ARCS")) %>%
...
ggplot(metrics) +
	...
	expand_limits(x = c(0, 2000), y = c(0, 15e6)) +
