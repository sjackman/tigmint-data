...
metrics <- quast_orig %>%
	...
	mutate(
		...
		Tigmint_ARCS = str_extract(Assembly, "Tigmint\\+ARCS|ARCS|Tigmint"),
		Tigmint_ARCS = ifelse(!is.na(Tigmint_ARCS), Tigmint_ARCS, Assembler))
```

# Plot NGA50 and breakpoints
```{r nga50-breakpoints, fig.width=6, fig.height=4, dpi=300}
ggplot(metrics) +
	aes(x = Misassemblies, y = Scaffold_NGA50, label = Tigmint_ARCS, shape = Assembler, colour = Assembler) +
