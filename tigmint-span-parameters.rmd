...
ggplot(metrics) +
	...
	scale_x_log10(breaks = unique(metrics$Window), minor_breaks = NULL) +
	theme_minimal(base_size = 20)
```
...
```{r NGA50-window}
ggplot(metrics) +
	aes(x = Window, y = Scaffold_NGA50, label = Span) +
	geom_point(aes(colour = Misassemblies)) +
	geom_smooth(method = "lm") +
	geom_text_repel() +
	scale_y_continuous(name = "QUAST Scaffold NGA50", labels = comma) +
	scale_x_log10(breaks = unique(metrics$Window), minor_breaks = NULL) +
	theme_minimal(base_size = 20)
```
...
ggplot(metrics) +
	...
	geom_point(aes(colour = Scaffold_NGA50)) +
	geom_smooth(method = "lm") +
	geom_text_repel() +
	scale_y_continuous(name = "QUAST Misassemblies", labels = comma) +
	scale_x_log10(breaks = unique(metrics$Span), minor_breaks = NULL) +
	theme_minimal(base_size = 20)
```
...
```{r NGA50-span}
ggplot(metrics) +
	aes(x = Span, y = Scaffold_NGA50, label = Window) +
	geom_point(aes(colour = Misassemblies)) +
	geom_smooth(method = "lm") +
	geom_text_repel() +
	scale_y_continuous(name = "QUAST Scaffold NGA50", labels = comma) +
